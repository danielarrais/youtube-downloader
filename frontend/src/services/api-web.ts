...
export const webApi: AppAPI = {
  ...
  removeDownload: (id, deleteFile) => request(`/api/downloads/${encodeURIComponent(id)}`, {
    method: 'DELETE',
  ...
  clearCompleted: (deleteFiles) => post('/api/queue/clear-completed', { delete_files: deleteFiles }),
  cancelAll: () => post('/api/queue/cancel-all'),
  pauseQueue: () => post('/api/queue/pause'),
  resumeQueue: () => post('/api/queue/resume'),
  clearAll: (deleteFiles) => post('/api/queue/clear-all', { delete_files: deleteFiles }),
