...
export const webApi: AppAPI = {
  ...
  removeDownload: (id) => request(`/api/downloads/${encodeURIComponent(id)}`, { method: 'DELETE' }),
  ...
  clearCompleted: (_deleteFiles) => post('/api/queue/clear-completed'),
  cancelAll: () => post('/api/queue/cancel-all'),
  pauseQueue: () => post('/api/queue/pause'),
  resumeQueue: () => post('/api/queue/resume'),
  clearAll: (_deleteFiles) => post('/api/queue/clear-all'),
